...
def test_user_can_select_a_custom_marker(test_runner, game_runner):
    ...
    assert game_runner.player_1.get_marker() == '!'


def test_user_can_select_a_custom_emoji_marker(test_runner, game_runner):
    test_runner.start_menu(['1', '2', 'thumbsup', '1', '1', '', 'n'], game_runner)

    assert game_runner.player_1.get_marker() == '👍'


def test_user_will_continue_to_be_prompted_if_they_provide_an_invalid_emoji_name(test_runner, game_runner):
    test_runner.start_menu(['1', '2', 'not_an_emoji', 'also_wrong', 'thumbsup', '1', '1', '', 'n'], game_runner)

    assert game_runner.player_1.get_marker() == '👍'


def test_a_user_will_continue_to_be_prompted_if_they_provide_an_integer_as_a_marker(game_runner):
    console = Console(MockConsoleIO(['1', '1', '5', '2', '7', '!', '1', '1', '', 'n']))
...
def test_user_cannot_select_the_same_marker_as_their_opponent(test_runner, game_runner):
    test_runner.start_menu(['1', '1', 'X', '2', '1', 'X', '1', 'O', 'n'], game_runner)
